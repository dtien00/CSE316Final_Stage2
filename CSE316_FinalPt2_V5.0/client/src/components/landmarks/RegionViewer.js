...
const RegionViewer = (props) => {
    ...
    const [changeParent, toggleParent] = useState(false);
    ...
    const toggleChangeParent = (e) => {
        console.log(changeParent);
        toggleParent(!changeParent);
        console.log("Value: " + e.target.value);
        // priorityRegion.parent = e.target.value;
        console.log(props.activeRegion.parent);
    }
	...
	return (
        ...
        {changeParent ? <select
                        className='region-viewer-parent-selection region-property' onBlur={toggleChangeParent}
                        autoFocus={true} defaultValue={priorityRegion.parent}
                    >
                        {props.activeMap.regions.map((entry, index)=>
                            (entry._id!=priorityRegion._id) ? <option value={entry.name}>{entry.name}</option>:null
                        )}
                    </select>
                    :
                    <label className="region-viewer-parent region-property">{"Parent Region: " + (priorityRegion.parent==' ' ? "None":priorityRegion.parent)}</label>
        }
        
        {!changeParent&&<button className="material-icons region-viewer-parent-changer" onClick={toggleChangeParent}>edit_outline</button>}
        ...
        <label className="region-landmarks-label">Region Landmarks:</label>
            ...
            <WInput 
				className="landmark-input" onKeyDown={addLandmarkKey} onBlur={updateInput} name="name" labelAnimation="fixed" 
...
// (priorityRegion.parent==' ' ? "None":priorityRegion.parent)