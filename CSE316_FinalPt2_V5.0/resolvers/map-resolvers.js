	...
	Mutation: {
		...
		**/
		deleteRegion: async (_, args) => {
			...
			console.log(mapRegions);
			const updated = await Map.updateOne({_id: _id}, { regions: mapRegions })
		...
		**/
		updateRegionField: async (_, args) => {
			...
			const { mapID, region_id, field, value } = args;
			let { data } = args
			const mapId = new ObjectId(mapID);
			const found = await Map.findOne({_id: mapId});
			let mapRegions = found.regions;
			console.log(mapRegions);
			let mapRegion = mapRegions.find((region) => region._id==region_id)

			console.log(mapRegion);
			mapRegion[field] = value;
			console.log(mapRegion);
		...
		**/
		...
		sortRegions: async (_, args) => {
			...
			let sortedRegions = [...found.regions];
			sortedRegions.sort((a,b) => a[field].localeCompare(b[field]));
			if(sortedRegions.toString()==found.regions.toString())	sortedRegions.reverse();

			console.log("ID: " + mapId);
			const updated = await Map.updateOne({_id: mapId}, {regions: sortedRegions});
			console.log("ORIGINAL " + found.regions);
		...
		addLandmark: async(_, args) => {
			const{map_id, region_id, landmark} = args;
			console.log("Adding new Landmark");
			const found = await Map.findOne({_id: map_id});
			let mapRegions = found.regions;
			console.log(mapRegions);
			...
			let mapRegion = mapRegions.find((region) => region._id == region_id);
			console.log(mapRegion);
			console.log("LANDMARKS: " + mapRegion.landmarks);
			mapRegion.landmarks.push(landmark);
			console.log("LANDMARKS AFTER: " + mapRegion.landmarks);
			console.log(mapRegion);
			
			const updated = await Map.updateOne({_id: map_id}, { regions: mapRegions });
		...
		deleteLandmark: async(_, args) => {
			...
			console.log(mapRegion);
			console.log("LANDMARKS: " + mapRegion.landmarks);
			mapRegion.landmarks.splice(landmark, 1);
			
			
			const updated = await Map.updateOne({_id: map_id}, { regions: mapRegions });
			if (updated) return region_id;
			return "Couldn't delete landmark";
		},

		editLandmark: async(_, args) => {
			const{map_id, region_id, landmark, newName} = args;
			console.log("Editing landmark name");
			const found = await Map.findOne({_id: map_id});
			let mapRegions = found.regions;
			console.log(mapRegions);
			console.log("Finding single");
			...
			console.log(mapRegion);
			console.log("LANDMARKS: " + mapRegion.landmarks);
			console.log(mapRegion.landmarks[landmark]);
			mapRegion.landmarks[landmark] = newName;
